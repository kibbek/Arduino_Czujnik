...
#define Light_topic "Czujnik/Stan/Komenda/"
#define MODE_OFF    0
#define MODE_ON     1
...
void loop() {
  ...
  switch (senseMode) {
    ...
    case MODE_ON:
      if (Serial.available() > 1) {
        incomingLux = Serial.read() ;
        incomingLux = incomingLux + Serial.read() * 256;
        client.publish("Czujnik/Lux/Raport", String(incomingLux).c_str(), true);
      }
      break;
  }
  ...
  if (millis() > (Time + 5000)) {
    ...
    client.publish("Czujnik/Stan/Raport", String(msgString).c_str(), true);
  }
}
